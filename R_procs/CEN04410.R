...
dalkar.meta <- tibble(
  dalkanofn_cd = c('byggingagerd', 'ibuatala', 'eignarhaldsform', 'thettseta', 'CEN04410.gildi'),
...
names(data.cd)[-length(names(data.cd))] <- paste(names(data.cd)[-length(names(data.cd))], 'cd', sep = '.')

names(data.is) <- dalkar.meta$dalkanofn_cd
names(data.is)[-length(names(data.is))] <- paste(names(data.is)[-length(names(data.is))], 'is', sep = '.')

names(data.en) <- dalkar.meta$dalkanofn_cd
names(data.en)[-length(names(data.en))] <- paste(names(data.en)[-length(names(data.en))], 'en', sep = '.')
